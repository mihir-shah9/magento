...
class Ccc_Vendor_Block_Adminhtml_Product_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
    ...
    protected function _prepareLayout()
    {
        ...
        if ($setId) {
            ...
            foreach ($groupCollection as $group) {
                ...
                $this->addTab('group_' . $group->getId(), array(
                    ...
                    'content' => $this->_translateHtml($this->getLayout()->createBlock(
                        ...
                        'adminhtml.product.edit.tab.attributes'
                    )->setGroup($group)
            ...
            //         'label' => Mage::helper('vendor')->__('Inventory'),
            //         'content' => $this->_translateHtml($this->getLayout()
            //             ->createBlock('vendor/product_edit_tab_inventory')->toHtml()),
            //     ));
            // }
            ...
            //         'label' => Mage::helper('vendor')->__('Websites'),
            //         'content' => $this->_translateHtml($this->getLayout()
            //             ->createBlock('vendor/product_edit_tab_websites')->toHtml()),
            //     ));
            // }

            //     $this->addTab('categories', array(
            //         'label' => Mage::helper('vendor')->__('Categories'),
            //         'url' => $this->getUrl('*/*/categories', array('_current' => true)),
            //         'class' => 'ajax',
            //     ));

            //     $this->addTab('related', array(
            //         'label' => Mage::helper('vendor')->__('Related Products'),
            //         'url' => $this->getUrl('*/*/related', array('_current' => true)),
            //         'class' => 'ajax',
            //     ));

            //     $this->addTab('upsell', array(
            //         'label' => Mage::helper('vendor')->__('Up-sells'),
            //         'url' => $this->getUrl('*/*/upsell', array('_current' => true)),
            //         'class' => 'ajax',
            //     ));

            //     $this->addTab('crosssell', array(
            //         'label' => Mage::helper('vendor')->__('Cross-sells'),
            ...
            //     $alertPriceAllow = Mage::getStoreConfig('product/productalert/allow_price');
            //     $alertStockAllow = Mage::getStoreConfig('product/productalert/allow_stock');

            //     if (($alertPriceAllow || $alertStockAllow) && !$product->isGrouped()) {
            //         $this->addTab('productalert', array(
            //             'label' => Mage::helper('vendor')->__('Product Alerts'),
            //             'content' => $this->_translateHtml($this->getLayout()
            //                 ->createBlock('vendor/product_edit_tab_alerts', 'admin.alerts.products')->toHtml())
            //         ));
            //     }

            //     if ($this->getRequest()->getParam('id', false)) {
            //         if (Mage::helper('vendor')->isModuleEnabled('Mage_Review')) {
            //             if (Mage::getSingleton('admin/session')->isAllowed('admin/catalog/reviews_ratings')) {
            //                 $this->addTab('reviews', array(
            //                     'label' => Mage::helper('vendor')->__('Product Reviews'),
            //                     'url' => $this->getUrl('*/*/reviews', array('_current' => true)),
            //                     'class' => 'ajax',
            //                 ));
            //             }
            //         }
            //         if (Mage::helper('vendor')->isModuleEnabled('Mage_Tag')) {
            //             if (Mage::getSingleton('admin/session')->isAllowed('admin/catalog/tag')) {
            //                 $this->addTab('tags', array(
            //                     'label' => Mage::helper('vendor')->__('Product Tags'),
            //                     'url' => $this->getUrl('*/*/tagGrid', array('_current' => true)),
            //                     'class' => 'ajax',
            //                 ));

            //                 $this->addTab('customers_tags', array(
            //                     'label' => Mage::helper('vendor')->__('Customers Tagged Product'),
            ...
            //             'label' => Mage::helper('vendor')->__('Custom Options'),
    ...
    public function getProduct()
    {
        if (!($this->getData('product') instanceof Ccc_Vendor_Model_Product)) {
            $this->setData('product', Mage::registry('vendor_product'));
        }
