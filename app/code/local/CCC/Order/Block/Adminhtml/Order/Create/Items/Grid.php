...
class Ccc_Order_Block_Adminhtml_Order_Create_Items_Grid extends Mage_Adminhtml_Block_Sales_Order_Create_Abstract
{
    /**
    ...
    public function getQtyTitle($item)
    {
        ...
        if ($prices) {
            ...
            foreach ($prices as $data) {
                ...
                $info[] = $this->helper('order')->__('Buy %s for price %s', $qty, $price);
            }
            return implode(', ', $info);
        } else {
            return $this->helper('order')->__('Item ordered qty');
        }
    }
    ...
    public function getTierHtml($item)
    {
        ...
        if ($prices) {
            foreach ($prices as $data) {
                $qty    = $data['price_qty'] * 1;
                $price  = $this->convertPrice($data['price']);
                $info[] = $this->helper('order')->__('%s for %s', $qty, $price);
            }
